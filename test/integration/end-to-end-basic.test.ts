...
describe('End-to-End Basic Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockSpreadsheet = {
      getSheetByName: jest.fn().mockImplementation((name: string) => {
        ...
        if (name === 'data') {
          return {
            getRange: jest.fn().mockReturnValue(createMockRange([
              [true, 'pending', 'テストタイトル', 'テスト説明', '高', '2025-08-20', 'テストユーザー', 'タグ1,タグ2', 'https://example.com', 'test@example.com']
            ])),
  ...
  describe('Trigger Processing', () => {
    it('should handle onEdit trigger correctly', () => {
      ...
      // onEditハンドラーをテスト
      expect(() => {
        triggerManager.onEdit(mockEvent);
      }).not.toThrow();
    });
