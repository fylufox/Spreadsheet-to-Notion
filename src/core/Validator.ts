...
export class Validator {
  ...
  public static validateRowData(
    ...
    try {
      ...
      if (rowData.length < CONSTANTS.COLUMNS.DATA_START) {
        errors.push(`Row data must have at least ${CONSTANTS.COLUMNS.DATA_START} columns`);
        return { valid: false, errors };
      }
      ...
      for (const mapping of targetMappings) {
        ...
        if (columnIndex === -1) {
          errors.push(`Column '${mapping.spreadsheetColumn}' not found in row data`);
          continue;
        }
  ...
  private static validateCellValue(
    ...
    switch (mapping.dataType) {
      ...
      default:
        errors.push(`Unknown data type '${mapping.dataType}' for field '${fieldName}'`);
        return { valid: false, errors };
    }
  }
  ...
  private static validateText(value: any, fieldName: string): ValidationResult {
    ...
    if (typeof value !== 'string' && typeof value !== 'number') {
      errors.push(`Field '${fieldName}' must be text or number, got ${typeof value}`);
    }
    
    const stringValue = String(value);
    if (stringValue.length > 2000) {
      errors.push(`Field '${fieldName}' exceeds maximum length of 2000 characters`);
    }
   ...
   * 数値型の検証
   */
  private static validateNumber(value: any, fieldName: string): ValidationResult {
    const errors: string[] = [];
  ...
  private static validateDate(value: any, fieldName: string): ValidationResult {
    ...
    if (value instanceof Date) {
      ...
      errors.push(`Field '${fieldName}' must be a valid date, got ${typeof value}`);
      return { valid: false, errors };
    }
    
    if (isNaN(dateValue.getTime())) {
      errors.push(`Field '${fieldName}' contains invalid date value '${value}'`);
    }
   ...
   * セレクト型の検証
   */
  private static validateSelect(value: any, fieldName: string): ValidationResult {
    const errors: string[] = [];
    
    if (typeof value !== 'string' && typeof value !== 'number') {
      errors.push(`Field '${fieldName}' must be text for select option, got ${typeof value}`);
    }
    ...
    if (stringValue.length > 100) {
      errors.push(`Field '${fieldName}' select option exceeds maximum length of 100 characters`);
    }
   ...
   * チェックボックス型の検証
   */
  private static validateCheckbox(value: any, fieldName: string): ValidationResult {
    const errors: string[] = [];
  ...
  private static validateUrl(value: any, fieldName: string): ValidationResult {
    ...
    if (typeof value !== 'string') {
      errors.push(`Field '${fieldName}' must be a valid URL string, got ${typeof value}`);
      return { valid: false, errors };
    }
    
    if (!CONSTANTS.PATTERNS.URL.test(value)) {
      errors.push(`Field '${fieldName}' must be a valid HTTP/HTTPS URL, got '${value}'`);
    }
    
    if (value.length > 2000) {
      errors.push(`Field '${fieldName}' URL exceeds maximum length of 2000 characters`);
    }
   ...
   * メール型の検証
   */
  private static validateEmail(value: any, fieldName: string): ValidationResult {
    const errors: string[] = [];
    
    if (typeof value !== 'string') {
      errors.push(`Field '${fieldName}' must be a valid email string, got ${typeof value}`);
      return { valid: false, errors };
    }
    
    if (!CONSTANTS.PATTERNS.EMAIL.test(value)) {
      errors.push(`Field '${fieldName}' must be a valid email address, got '${value}'`);
    }
    
    if (value.length > 320) {
      errors.push(`Field '${fieldName}' email exceeds maximum length of 320 characters`);
    }
   ...
   * 電話番号型の検証
   */
  private static validatePhoneNumber(value: any, fieldName: string): ValidationResult {
    const errors: string[] = [];
    
    if (typeof value !== 'string' && typeof value !== 'number') {
      errors.push(`Field '${fieldName}' must be a valid phone number, got ${typeof value}`);
      return { valid: false, errors };
    }
    ...
    if (!CONSTANTS.PATTERNS.PHONE.test(stringValue)) {
      errors.push(`Field '${fieldName}' must be a valid phone number format, got '${value}'`);
    }
    
    if (stringValue.length > 50) {
      errors.push(`Field '${fieldName}' phone number exceeds maximum length of 50 characters`);
    }
  ...
  public static validateMapping(mapping: ColumnMapping): ValidationResult {
    ...
    // データ型の妥当性チェック
    if (mapping.dataType && !Object.values(CONSTANTS.DATA_TYPES).includes(mapping.dataType as any)) {
    ...
    if (mapping.notionPropertyName) {
      if (mapping.notionPropertyName.length > 100) {
        errors.push('Notion property name exceeds maximum length of 100 characters');
      }
      
      // 予約語チェック
      const reservedWords = ['id', 'created_time', 'last_edited_time', 'created_by', 'last_edited_by'];
      if (reservedWords.includes(mapping.notionPropertyName.toLowerCase())) {
        errors.push(`'${mapping.notionPropertyName}' is a reserved property name`);
      }
    }
  ...
  public static validateMappings(mappings: ColumnMapping[]): ValidationResult {
    ...
    mappings.forEach((mapping, index) => {
      ...
      if (!result.valid) {
        errors.push(...result.errors.map(error => `Mapping ${index + 1}: ${error}`));
      }
    });
    
    // 重複チェック
    const spreadsheetColumns = mappings.map(m => m.spreadsheetColumn?.trim()).filter(Boolean);
    const notionProperties = mappings.map(m => m.notionPropertyName?.trim()).filter(Boolean);
    ...
    if (duplicateSpreadsheetColumns.length > 0) {
      errors.push(`Duplicate spreadsheet columns: ${duplicateSpreadsheetColumns.join(', ')}`);
    }
    
    if (duplicateNotionProperties.length > 0) {
      errors.push(`Duplicate Notion properties: ${duplicateNotionProperties.join(', ')}`);
    }
    
    // タイトル型の存在チェック
    const titleMappings = mappings.filter(m => m.dataType === CONSTANTS.DATA_TYPES.TITLE && m.isTarget);
   ...
   * データ変換可能性のチェック
   */
  public static canConvertToNotionType(value: any, targetType: string): boolean {
    if (this.isEmpty(value)) {
    ...
    try {
      switch (targetType) {
        ...
        case CONSTANTS.DATA_TYPES.URL:
          return typeof value === 'string' && CONSTANTS.PATTERNS.URL.test(value);
          
        case CONSTANTS.DATA_TYPES.EMAIL:
          return typeof value === 'string' && CONSTANTS.PATTERNS.EMAIL.test(value);
